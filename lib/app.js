...
App.prototype.render = function(view, data, callback) {
  ...
  var render = this.info.engines[ext] || this.info.engines[''];
  try {
    (render.length === 4) ? render(view, data, { debug:!!this.get('view debug'), beautify:!!this.get('view debug'), mangle:false }, callback) : render(view, data, callback);
  } catch(ex) {
    return callback(ex);
  }
};
