...
function App(originalReq, originalRes, callback) {
  ...
  originalRes.toString = function() {
    return '[ object ServerResponse ]';
  };
  originalReq.toString = function() {
    return '[ object ServerRequest ]';
  };
  var req = Request(originalReq);
  var res = Response(this, originalRes, req);
  ...
  function prepare(callback) {
    callback(undefined, undefined, req, res);
  }
  var handlers = this.routes.map(function(route, ridx) {
    return function(err, req, res, callback) {
      if(!dom) return callback(new Error('Domain Gone'));
      this.trace('start:route(' + ridx + ')', req.URL.pathname);
      var myCallback = req.nextCallback = dom.bind(function(err, request, response) {
        this.trace('done:route(' + ridx + ')', req.URL.pathname);
...
App.create = function() {
  var app = function(req, res, callback) {
...
App.inherit = function(from, constr) {
  /*jslint forin:true */
  var name;
  for(name in from.prototype) {
    this[name] = from.prototype[name];
  }
  return(constr || from).apply(this, Array.prototype.slice.call(arguments, 3));
};

App.prototype.mount = function(prefix) {
  prefix = String(prefix);
  var handler = function(req, res, unmount) {
    if(req.URL.pathname.substr(0, prefix.length) !== prefix) {
      return unmount();
    }
    req.URL.pathname = req.URL.pathname.substr(prefix.length);
    this.call(this, req, res, callback);

    function callback(err, req, res) {
      req.URL.pathname = prefix + req.URL.pathname;
      unmount(err, req, res);
    }
  }.bind(this);
  handler.toString('[ object MacadamiaApp(' + prefix + ') ]');
  return handler;
};
...
App.prototype.POST = App.prototype.post = App.prototype.pst = function(matcher, handler) {
...
App.prototype.DELETE = App.prototype.delete = App.prototype.del = function(matcher, handler) {

  return this.route('DELTE', matcher, handler);
};
App.prototype.OPTIONS = App.prototype.options = App.prototype.opt = function(matcher, handler) {

  return this.route('OPTIONS', matcher, handler);
};
...
App.prototype.prop = function(name, value) {
  if(undefined !== value) {
    if(null === value) {
      delete this[name];
    } else {
...
App.prototype.render = function(renderpath, data, options, callback) {
  ...
  this.first(Object.keys(this.engines).map(function(item) {
    return base.concat([item]).join('.');
  }.bind(this)), fs.stat, function(renderpath) {
    renderpath = renderpath || base.join('');
    this.trace('render:engine(' + renderpath + ')');
    var chosen = this.engines[path.extname(renderpath).substr(1)];
...
App.prototype.merge.copy = function(res, one, key) {
  if(('object' === typeof one[key]) && (null !== one[key])) {
    if(Array.isArray(one[key])) {
      ...
      res[key] = this.merge(res[key], one[key]);
    }
  } else if((undefined !== one[key]) && (null !== one[key])) {
    res[key] = one[key];
  }
};
...
App.prototype.error = function(err, code) {
  err = ('object' !== typeof err) ? new Error(err) : err;
  err.status = err.status || code || 500;
  return err;
};
App.prototype.assert = function(cond, msg, code, callback) {
  if(cond) return true;
  if(('function' === typeof code) && !callback) {
    callback = code;
    code = 500;
  }
  code = isNaN(code) ? 500 : code;
  var err = this.error(msg, code);
  if('function' === typeof callback) return callback(err) && false;
  throw err;
};
