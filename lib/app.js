...
App.prototype.handle = function(req, res) {
  ...
  async.waterfall([ prepare ].concat(app.context.routes.map(router)), dom.bind(errorHandler));
  ...
  function router(route) {
    return dom.bind(function(err, req, res, callback) {
      try {
        route.call(app, err, req, res, function(err) { callback(undefined, err, req, res); });
      } catch(ex) {
        callback(ex);
      }
    });
