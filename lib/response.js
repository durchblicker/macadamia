...
function MacadamiaResponse(response, request, dontAttach) {
  Writable.call(this);
  Object.defineProperty(this, 'wrapped', { value:response });
  Object.defineProperty(this, 'request', { value:request||response.request });

  Object.defineProperty(this, 'time', {
    get:('undefined' === typeof response.time) ? time.bind(this, process.hrtime()) : getter.bind(null, response,'time'),
    enumerable:true
  });
  ...
  Object.defineProperty(this, 'bodySent', {
    get:getter.bind(null, response,'bodySent'),
    set:setter.bind(null, response,'bodySent'),
    enumerable:true
  });
  if (!response.transformer) response.on('finish', setter.bind(null, this, 'bodySent', true));

  Object.defineProperty(this, 'transformer', { value: [ new Transformer() ], enumerable:true });
  ...
  this.on('finish', function() {
    if (!this.headersSent) finalizeTransformers.call(this, response, this.transformer);
    this.transformer[0].end();
  }.bind(this));
...
MacadamiaResponse.prototype = new Writable();
...
MacadamiaResponse.prototype.size = function(size) {
  isNaN(size) ? this.del('Content-Length') : this.setHeader('Content-Length', size);
  return this;
};
...
MacadamiaResponse.prototype.toString = function() {
  ...
  return '[ object MacadamiaResponse ]';
};

function finalizeTransformers(ctx, transformer) {
  (function(array) {
    function immutable() { throw new Error('once writing has started MacadamiaResponse.transformer is immutable'); };
    array.push=immutable;
    array.pop=immutable;
    array.shift=immutable;
    array.unshift=immutable;
    array.sort=immutable;
    array.reverse=immutable;
    array.splice=immutable;
  }(transformer));
  transformer.forEach(function(item, idx, all) {
    var dst = (idx < (all.length-1)) ? transformer[idx+1] : ctx;
    //console.error(item+'.pipe('+dst+')');
    item.pipe(dst);
  }.bind(this));
  transformer[0].on('drain', this.emit.bind(this, 'drain'));
}
function write(ctx, chunk, callback) {
  if (!this.headersSent) {
    ...
    this.emit('body');
    finalizeTransformers.call(this, ctx, this.transformer);
  }
  //console.error(this+'.write('+chunk.length+') -> '+this.transformer[0]);
  return this.transformer[0].write(chunk, callback);
}
...
function getter(ctx, name) {
  //console.error('Getting: '+name+' = '+ctx[name]);
  return ctx[name];
}
function setter(ctx, name, value) {
  //console.error('Setting: '+name+' to '+value);
...
Transformer.prototype = new Transform();
Transformer.prototype._transform = function(chunk, write, callback) {
  write(chunk);
  callback();
};
