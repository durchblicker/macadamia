...
function wrap(app, response, request) {
  if(response instanceof MacadamiaResponse) return response;
  return new MacadamiaResponse(app, response, request);
}

function MacadamiaResponse(app, response, request) {
  ...
  Object.defineProperty(this, 'data', {
    ...
    set: function(data) {
      ctx.data = this.application.merge(ctx.data, data);
    }.bind(this),
    enumerable: true
  });
  ...
  Object.defineProperty(this, '_write', {
    value: write.bind(this, response),
    enumerable: false,
    configurable: true
  });
  Object.defineProperty(this, 'writeContinue', {
    value: writeContinue.bind(this, response),
    enumerable: true,
    configurable: true
  });
  Object.defineProperty(this, 'setHeader', {
    value: setHeader.bind(this, response),
    enumerable: true,
    configurable: true
  });
  Object.defineProperty(this, 'getHeader', {
    value: getHeader.bind(this, response),
    enumerable: true,
    configurable: true
  });
  Object.defineProperty(this, 'addHeader', {
    value: addHeader.bind(this, response),
    enumerable: true,
    configurable: true
  });
  Object.defineProperty(this, 'removeHeader', {
    value: removeHeader.bind(this, response),
    enumerable: true,
    configurable: true
  });
  Object.defineProperty(this, 'addTrailers', {
    value: addTrailers.bind(this, response),
    enumerable: true,
    configurable: true
  });
  ...
  this.on('finish', function() {
    ctx.bodySent = true;
    this.application.trace('done:body', this.request.URL.pathname);
    response.end();
  }.bind(this));
...
MacadamiaResponse.prototype.type = function(type) {
  if(!type) return(this.getHeader('Content-Type') || '').replace(/;\s[\s|\S]+$/, '');
...
MacadamiaResponse.prototype.cookie = function(name, value, options) {
  ...
  value = [
    [name, value].join('='), ['Path', options.path].join('=')];
...
MacadamiaResponse.prototype.json = function(data) {
  this.status(this.statusCode || 200).type('json');
...
MacadamiaResponse.prototype.html = function(data) {
  this.status(this.statusCode || 200).type('text/html');
  return this.send(data);
};
MacadamiaResponse.prototype.render = function(path, data, options, callback) {
  if(('function' === typeof data) && !options && !callback) {
    callback = data;
    data = {};
    options = {};
  }
  ...
  assert('function' === typeof callback, this + '.render needs a callback');
  data = this.application.merge(this.data, data);
  this.application.render(path, data, options, function(err, content) {
    if(!err) this.send(content);
    return callback(err);
  }.bind(this));
};
...
function write(ctx, chunk, callback) {
  if(!this.headersSent) {
    ...
    this.emit('body');
    this.application.trace('start:body', this.request.URL.pathname);
  }
  var res = ctx.write(chunk, callback);
  this.application.trace('need:drain(' + chunk.length + ')' + (!res), this.request.URL.pathname);
  return res;
}
...
function writeHead(ctx, status, reason, headers) {
  ...
  headers = headers || ('object' === typeof reason ? reason : headers) || {};
...
function addHeader(ctx, name, value) {
  ...
  current = ((undefined === current) || (null === current)) ? [] : (Array.isArray(current) ? current : [current]);
