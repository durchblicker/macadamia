...
function Response(app, res) {
  ...
  Object.keys(Response.prototype).forEach(function(name) {
    obj[name] = Response.prototype[name];
  });
  return obj;
}
Response.prototype.status = function(code) {
  if (this._header) return this;
...
Response.prototype.location = function(location) {
  if (location==='back') {
    location=this.request.get('referer') || this.request.URL.resolve('/');
  } else {
...
Response.prototype.writeHead = function(status, reason, headers) {
  ...
  Object.defineProperty(this,'headersSent', { value:true, enumerable:true });
  var res = this;
  ...
  this.emit('header');
  return this.response.writeHead(status, reason);
};
Response.prototype.write = function(data, encoding) {
  if (this.bodySent) return this;
  if (!this.headersSent) this.writeHead(this.response.statusCode);
...
Response.prototype.error = function(status, message) {
  status = status || 500;
