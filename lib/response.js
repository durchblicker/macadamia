...
function sendJSON(json) {
  ...
  return this.type('application/json').size(json.length).send(json);
}
...
function cookie(name, value, options) {
  ...
  if(options.path) value.push(['Path', options.path].join('='));
...
function render(view, data, callback) {
  ...
  this.app.render(view, data, callback);
  return this;
}
